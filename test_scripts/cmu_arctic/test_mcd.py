...
def main():
    ...
    for src_file in tqdm(orig_list):
        filename, src_spk = re.match('(\S+)_cmu_us_(\S+)_arctic-X_orig.wav',os.path.basename(src_file)).groups()
        
        if src_spk not in dists.keys():
            dists[src_spk] = {}
        #print(filename,src_spk)
        conv_list =  glob.glob(os.path.join(args.test_path, f'{filename}_cmu_us_{src_spk}_arctic-*_conv.wav'))
        #print(conv_list)
        for conv_file in conv_list:
            #print(f'{filename}_{src_spk}-(\S+?)_conv.wav',conv_file)
            tgt_spk = re.match(f'{filename}_cmu_us_{src_spk}_arctic-cmu_us_(\S+)_arctic_conv.wav',os.path.basename(conv_file)).group(1)
            
            if tgt_spk not in dists[src_spk].keys():
                dists[src_spk][tgt_spk] = []
            
            tgt_file = os.path.join(args.test_path,f'{filename}_cmu_us_{tgt_spk}_arctic-X_orig.wav')
    ...
    dists_orig={}      
    for src_file in tqdm(orig_list):
        filename_src, src_spk = re.match('(\S+)_cmu_us_(\S+)_arctic-X_orig.wav',os.path.basename(src_file)).groups()
        if src_spk not in dists_orig.keys():
            dists_orig[src_spk] = {}
        #print(filename_src,src_spk)
        for tgt_file in orig_list:
            filename_tgt, tgt_spk = re.match('(\S+)_cmu_us_(\S+)_arctic-X_orig.wav',os.path.basename(tgt_file)).groups()
    ...
    results = {'test_mcd':dists, 'ref_mcd':dists_orig}

...
def build_audio_and_result_table(phrase_id, phrase_idx, spks, result_dict):
    ...
    table += '<tr>\n<td>Originals</td>\n'
    for tgt_spk in spks:
        table += f'<td><audio id="player" controls preload="none"><source src="signals/arctic_{phrase_id}_cmu_us_{tgt_spk}_arctic-X_orig.wav" /></audio></td>\n'
    table += '</tr>\n'   
...
def build_result_table(phrase_id, spks, result_dict):
    ...
    for src_spk in spks:
        ...
        for tgt_spk in spks:
            table += f'<td>{result_dict[src_spk][tgt_spk][pid]:.2f}</td>\n'
        table += '</tr>\n'
    return html_table_template.format(table)
...
def build_html(out_filename, test_dir):
    ...
    for f in filelist:
        ...
        phrase_id, src_spk = re.match(r'arctic_(\S+)_cmu_us_(\S+)_arctic-X_orig.wav',os.path.basename(f)).groups()
